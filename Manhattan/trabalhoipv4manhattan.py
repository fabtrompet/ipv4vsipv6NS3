...
ripRouting.ExcludeInterface(routeadoreslist[int(cmd.numroteadores)-1],6);
...
address.SetBase(ns.network.Ipv4Address("192.168.40.0"), ns.network.Ipv4Mask("255.255.255.0"))
...
for i in clienteslist:
	...
	static.AddHostRouteTo(ns.network.Ipv4Address("192.168.40.2"),ns.network.Ipv4Address("192.168.1."+str(int(cmd.numeroclientes)+1)),1);

static = ns.internet.Ipv4StaticRoutingHelper();
static = static.GetStaticRouting(s1.GetObject(ns.internet.Ipv4.GetTypeId()));
cont=1
for i in clienteslist:
	static.AddHostRouteTo(ns.network.Ipv4Address("192.168.1."+str(cont)),ns.network.Ipv4Address("192.168.40.1"),1);
	cont+=1
